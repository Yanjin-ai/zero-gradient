...
**F2-aux-zeroBP（`f2_aux.py`）**：给 ZeroBP 局部规则喂显式结构目标（3 类关系头，闭式 CE，dh 经局部规则更新 experts+embedding）。NLI zero-shot **51.3% → 51.8%（+0.5pp）**。**【事实】ZeroBP 即便喂结构目标也刻不出关系几何**——这是第 3 次 ZeroBP 失败（数据 +2pp / 结构目标 +0.5pp / Track A attention 局部规则）。**ZeroBP 通往关系结构的路线已穷尽。**

**H1-attn-hybrid（`h1_attn.py`）**：少量 BP 塑形（autograd 只碰 embedding/attention/头，experts/路由/底层仍 ZeroBP），再用**新鲜闭式头**读 NLI zero-shot（测"表示"是否得到结构，而非头拟合）。

| 塑形 | NLI zero-shot（新鲜头） |
|---|---|
| 无（F1 base） | 51.3% |
| **BP embedding** | **58.8%（+7.5pp）** |
| BP embedding+attention | 59.1%（+0.3pp） |

**【事实】少量 BP 确实把关系结构装进了表示**（51→59%），而三条 ZeroBP 路线都 ≤+2pp → **墙的本质是 BP vs ZeroBP**。但**公平新鲜头读出下，是 embedding 在扛**，attention 只 +0.3pp（此前"attention 帮 NLI 到 62%"部分是 BP 头拟合、非表示）。且 59% **有限**（不到 65%+），与 NLI 4B chance 一致——**关系对齐对这套架构（弱 attention + 末位塌缩）在尺度上是真正难的**。

> **🔒【Phase F 关系结构子结论（锁定）】** ① ZeroBP 路线（数据 / 结构目标 / attention 局部规则）**都无法**给表示装入关系几何（≤+2pp）；② 少量 BP **能**部分装入（小配置 51→59%），墙是 **BP-vs-ZeroBP**；③ 但**embedding 是杠杆，attention 加成微弱**（公平读出），且**有限、不转移 4B**（NLI 4B 仍 chance）。→ **关系/多步结构是当前架构的真实上限；要靠拢现代 LLM 的这类能力，光"少量 BP 介入"不够，需要更强的结构归纳偏置（可训练 attention 本身 / 不塌缩到末位的序列读出 / 更深 BP），这超出"保持 ZeroBP backbone + 少量顶层 BP"的边界。**
