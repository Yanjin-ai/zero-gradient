...
- **【PROPOSAL】二选一**：(A) **收口** ADR-002 并转向**巩固已成立能力**（情感 79% / LM 1355 的展示线）；(B) **换骨架** = Phase H 新栈（真正多层可训练注意力 + 非 reservoir base），立**新 ADR**，小配置初验——超出当前 v2.0，谨慎。

---
**接棒一句话**：提交版是**干净的纯 ZeroBP 4B**（已锁定、已修复）；研究已测清 **ZeroBP 的关系/多步能力上限**（少量 BP 部分有效但 **embedding 主导**、不转移 4B）。Phase G 三条 v2.0 结构线已**全部测到头**：**不塌缩读出**证伪、**可训练 attention 隔离**+0.0pp 证伪、**更深 BP** = 关系小配置有信号（→65.7%）但不转移 4B、多步任何深度仍 chance。在当前骨架上加 BP 已无新增空间——下一步是**收口巩固已成立能力**或**换骨架（Phase H 新栈，需新 ADR）**。**勿改已锁定结论；勿破坏默认 6.251。**
