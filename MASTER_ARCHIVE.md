...
- **【诚实】G2b（生成式 GSM8K）+ Track1 SST-2**：kernel push 失败（Kaggle "Notebook not found"），且诊断中误升级 kaggle CLI 弄坏本地环境——**未跑**，待修 CLI 后重试。
- **【PROPOSAL】下一步**：修 CLI → 重跑 G2b/SST-2；推深多步（更大规模）；G1 也可扩到 MNLI。
