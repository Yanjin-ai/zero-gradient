...
- **【负结果·存疑】Track1 SST-2（ZeroBP 4B，GPU，DONE）**：真实 GLUE/SST-2 zero-shot/emb/emb+attn **全 49.08%**（<majority 50.9%），WikiText ppl 1355→1350.8（BP 确跑）。三变体逐位相同 → 疑**读出坍缩 artifact**，**未锁为干净数**；需 sanity-check SST-2 读出。对照合成 bag-sentiment 79%：真实词汇情感远更难。
- **【PROPOSAL】下一步**：sanity-check SST-2 读出（是否退化）；推深多步（更大规模，映射 k≥4 上限）；G1 扩 MNLI；G2b 若认真做需生成式规模化（超小 char LM）。
