...
- **【作用域 / 诚实边界】** G0 = **合成、小模型、完全可学**；**未**触及真实 SNLI/MNLI/GSM8K（需 GPU = G1/G2）。G0 过的意义 = "新栈有腿、值得上 GPU"，**不等于**已对齐现代 LLM。
- **【PROPOSAL】下一步**：G1（GPU）真实 NLI；G2（GPU）GSM8K 部分题；BP 深度阶梯跨栈对照。本地 CPU 跑不了，需按现有 Kaggle 流程上 GPU。
