...
## 5. 架构发现：末位塌缩 — LOCKED FACT（机制）+ INTERPRETATION（影响）
- **【LOCKED FACT（代码事实）】** `context()` 把整个序列塌缩成**最后一个位置**一个向量 `(emb+att@emb)[:, -1]`，之后所有 MoE block 都在这**一个** [B,d] 向量上算。
- **【INTERPRETATION】** 对关系/对齐/多步任务，跨位置信息很可能在**塌缩时丢失**——这比"attention 弱/BP 不够"更根本，是关系任务难的**疑似真因**（也解释 attention BP 公平读出只 +0.3pp）。
