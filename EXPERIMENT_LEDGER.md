...
| **🎯 G2 多步深度扫描（GPU，DONE）** | Kaggle T4，6L×8H d256 4.7M，6000 步/深度：**k=2 100%**、k=4 22.5%、k=6 19.9%、k=8 20.6%（chance 20%）。→ [FACT] **新骨架装得进 2 步（ZeroBP 任何 BP 深度连 2 步都 chance）；但此规模/训练下 k≥4 仍 chance**——深多步需更大规模/更长训练（本地 k=3 已证 2.67M/6000 步可达 100%，故 k≥4 是欠容量非绝对墙）。诚实：**深多步 GPU 上未装**，是 Phase H 下一个要推的边界 | `phase_h/ph_gsm_gpu.py`（kernel `post-backprop-phase-h-multi-step`） |
...
| **Kaggle 自动跑（部分成功）** | G1+G2 **DONE**（上表）。G2b+SST-2 push **失败**="Kernel push error: Notebook not found"（Kaggle 侧，疑与先触发"限 2 并发 GPU"后的状态有关；待重试）。**注意**：诊断中误运行 `pip install -U kaggle`→2.2.2+kagglesdk 0.1.32（缺 `competitions.legacy` 子模块的打包 bug）**弄坏了本地 kaggle CLI**；auth=`~/.kaggle/access_token`（仅 2.2.x 支持）。CLI 需修复才能续跑 G2b/SST-2 | `orchestrate_ph.py` · `orchestrate_kaggle.py sst2` |
