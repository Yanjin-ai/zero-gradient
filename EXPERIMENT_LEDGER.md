...
| **⚠ Track 1 SST-2（ZeroBP 4B，GPU，DONE 但存疑）** | 真实 GLUE/SST-2：zero-shot / Mixed-BP(emb) / (emb+attn) **全 49.08%**（428/872，< majority 50.9%）；WikiText ppl 1355→1350.8（BP 确跑了）。→ **负结果**：真实 SST-2 上 ZeroBP 栈 + 少量 BP = chance，且 3 个变体**完全相同**。[疑] 三变体逐位相同像**读出坍缩 artifact**（闭式头对 872 验证例全同 argmax）——**未锁为干净数**，需 sanity-check SST-2 读出路径。对照合成 bag-sentiment 79%：真实词汇情感远难于合成 | `track1_sst2_4b.py`（kernel `post-backprop-track1-sst2-v2`） |
